...
floating-point values than otherwise possible~\cite{gmp_paper, mattcomp_precision}.
...
DFT of length $N = RC$ can be computed as follows~\cite{mattcomp_mfa}:
\begin{enumerate}
...
Now, the case of inverse transform.
We now need to multiply each matrix element indexed $\langle i,j \rangle$
...
The x86 and x86-64 platforms provide ``conditional moves'' --- instructions that
...
It is possible the built-in two-word types: \verb~uint64_t~ on 32-bit systems and
\verb~unsigned __int128~ on 64-bit systems. But the latest versions of both Clang and GNU GCC do no
optimize the division of two-word types by a constant, as they do for one-word types. Thus,
this built-in division is slow.
