...
\begin{abstract}
    ...
    (2) a simple cache-efficient algorithm for in-place $2n \times n$ or $n \times 2n$ matrix transposition, the need for which arises in the ``six-step algorithm'' variation of the matrix Fourier algorithm, and which does not seem to be widely known.
...
Unfortunately, this method is very slow; in our benchmark, native division on x86-64
was 720\% slower than the Montgomery reduction. The code can be found in the
\verb!bench-modmul! subdirectory of our repository.
...
elements, is known as Barrett reduction. % TODO cite
...
$\approx$ 15\% compared to Montgomery reduction;
...
But note this does not mean just one of them can be omitted but not the other!
...
But on x86 and x86-64, there is difference:
these platforms can encode SIB (scale-index-base) directly in the instruction that dereferences the
...
memory in strides of $R$ or $C.$ Thus, it might be faster in settings of hierarchical memory,
...
matrix.
For completeness, we provide the code below.
...
There is, then, a result which says that
\begin{equation}
...
1 + \gcd(N_1-1, N_2-1).
$$
The number of cycles of length $k>1$ of $\mathcal{P}$ is given by
$$
...
even more complex algorithms are genreally needed;
...
After all, the case of six-step FFT with its
...
We provide the pseudocode for case of $C=2$ below.
...
Observe that steps 1 and 2 of the direct transform, namely
\begin{itemize}
\item length-3 DFT on each column,
\item multiplication by $(\xi_N)^{ij}$),
\end{itemize}
as well as steps 3 and 4 of the inverse transform, namely
\begin{itemize}
\item multiplication by $(\xi_N)^{-ij},$
\item length-3 IDFT on each column,
\end{itemize}
can be merged (performed in one pass over the columns).
In fact, they should be merged, as it produces less loads/stores and leads to better
cache utilization.
...
Note that we do need to multiply everything by $N^{-1}$ after performing the inverse transform
(or by another factor if the linearity trick is used; see section~\ref{section:trick}).
...
This operation on a signle column with index $j,$
$\langle x, y, z \rangle \mapsto \langle x', y', z' \rangle,$ can be expresed as:
...
Suppose now we need to multiply each matrix element indexed $\langle i,j \rangle$
...
We need to factorize $N = 2^k$ into $N_1 \cdot N_2 = N$ so that $\max\{N_1, N_2\}$ is as small as
possible.
This is because we will perform transforms of lengths both $N_1$ and $N_2,$ and we want to achieve
good cache utilization.
This is done by choosing $N_1 = 2^{\lfloor k / 2 \rfloor}, N_2 = 2^{\lceil k / 2 \rceil}.$
...
$c_i + \sigma_i$ is going to be closer to $\mu^2 / 4$ than to $\mu^2 / 2;$ but this is still a
tight bound in the sense that the maximum possible $c_i + \sigma_i$ would overflow one machine word.
This bound also has the advantage that it works for any $B > 1.$

\subsection{Shenanigans with Montgomery reduction}

Remember the procedure $\mathrm{MontgomeryReduce}(a)$ that we defined in section~\ref{section:montgomery_intro}.

\subsection{Cache aliasing problems}
