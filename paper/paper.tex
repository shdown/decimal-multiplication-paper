...
Split $M$ into two $n$-by-$n$ submatrices, left submatrix $A$ and right submatrix $B$:
$$
M = \begin{bmatrix}
A & B
\end{bmatrix}.
...
We write $\longleftrightarrow$ for ``equivalent in flat row-major order to''.
Then,
$$
\begin{bmatrix}
    A^T & B^T
...
So we can propose the following algorithm for transposing $M,$ although quite inefficient:
\begin{enumerate}
...
$\pi'' Y,$ where $Y$ is a matrix with $N$ columns, means $Y$ with columns permuted by $\pi.$
...
Define the result of transposition of both left and right submatrices of a $n$-by-$2n$ matrix $M$ as
$\Psi(M).$ Note this is step 1 of our algorithm. We have:
$$\rho' \Psi(M) = M^T.$$
...
It means that we can apply $\rho$ to \textit{columns} (as opposed to rows) \textit{before} the transpositions of
submatrices (as opposed to after) --- and still get the transposed matrix as the result.
...
to columns, then transpose the submatrices. Application of $\rho^{-1}$ to a column can also be done in a
...
It requires $C n$ additional memory, which, for fixed $C,$ is $O(\sqrt{N}),$ where
$N = C n^2$ is the number of elements in the matrix.
...
tight bound in the sense that maximum possible $c_i + \sigma_i$ would not into one machine word.
