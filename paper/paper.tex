...
For example, virtually all partical implementations of the ``fancy'' sorting algorithms that are divide-and-conquer in nature --- quick sort and merge sort --- do in fact fall back to ``dumb'' quadratic sorts if the size of the input is less than some threshold.
...
\begin{itemize}
    ...
    \item IDFT* means IDFT without multiplication by $N^{-1}.$
...
We provide the pseudocode for the case of $C=2.$
...
allowing non-local propagation of ``unadjustment'' would be a nightmare from the standpoints of
...
Define ``threshold input size'' for some fixed implementation as any value of $n$ where a
