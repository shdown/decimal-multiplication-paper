...
Remember the algorithm for computing $\redc^\ast(t) = R^{-1} t$
in $\mathrm{F}_p,$
assumes $$0 \le t < p \mu,$$ where $\mu = 2^w$ if the machine word is $w$ bits long.
We can then compute $\redc(x,y),$ where $0 \le x,y < p$, as $\redc^\ast(xy).$

Remember also that we require $p < \mu/2.$
