...
The four-step algorithm is just a restatement, in terms of a $N_1 \times N_2$ matrix, of the
...
Thus, it can be used when it is desirable to perform a transform of length $N$ which is not a
...
memory in strides of $R$ or $C.$ Thus, it might be faster in settings of hierarchial memory,
including external-memory transforms and modern systems with multiple levels of cache.

Remember our definition of the cyclic convolution in terms of DFT and IDFT:
$$x \star y = \mathfrak{f}^{-1}\big( \mathfrak{f}(x) \cdot \mathfrak{f}(y) \big),$$
where $\cdot$ denotes scalar (element-wise) product.

Note that, for computation of $\star,$ the specific order in which the direct transform
$\mathfrak{f}$ produces its outputs, and the order in which the inverse transform
$\mathfrak{f}^{-1}$ expects its inputs to be in, does not matter; we only need these two orders
to match.
Indeed, $\cdot$ is element-wise, $(x \cdot y)[i] = x[i] y[i],$
so, for any permutation $\pi$ we have $$\pi^{-1}(\pi(x) \cdot \pi(y)) = x \cdot y.$$
This means that, for the purpose of computing the cyclic convolution, in both four-step and six-step
algorithms, the final transposition step in DFT and the initial transposition step in IDFT, can be
simultaneously omitted. Note this does not mean you can omit just one of them but not another!
