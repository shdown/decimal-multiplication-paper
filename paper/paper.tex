...
The value of $p'$ can be pre-calculated with extended Euclidean algorithm for GCD;
we have $\gcd(p, \mu) = 1,$ so $\exists A, B \in \mathbb{Z} \colon A p + B \mu = 1.$
This means $A p \equiv 1 \, (\mymod \mu)$ and $-Ap \equiv -1 \, (\mymod \mu),$ so
$$p' = (-Ap) \mymod \mu.$$
...
If we express this transform as a function $\tau(A,\omega),$ then DFT and IDFT can be expressed in
terms of $\tau$ as follows:
\begin{eqnarray*}
\mathfrak{f}(x) &=& \tau(x, \xi); \\
\mathfrak{f}^{-1}(x) &=& N^{-1} \tau(x, \xi^{-1}),
\end{eqnarray*}
where $N$ is the length of the transform.
...
Remember we expressed the cyclic convolution in terms of DFT and IDFT as follows:
$$x \star y = \mathfrak{f}^{-1}\big( \mathfrak{f}(x) \cdot \mathfrak{f}(y) \big),$$
where $\cdot$ denotes scalar (element-wise) product.

Note that, for the computation of $(\_ \star \_),$ the specific order in which the direct transform
$\mathfrak{f}$ produces its outputs, and the order in which the inverse transform
$\mathfrak{f}^{-1}$ expects its inputs to be in, do not matter; we only need these two orders
to match.
Indeed, $(\_ \cdot \_)$ is element-wise, $(x \cdot y)[i] = x[i] y[i],$
so, for any permutation $\pi$ we have $$\pi^{-1}(\pi(x) \cdot \pi(y)) = x \cdot y.$$
...
$T_\ast[\Delta_s + j],$ where $\Delta_s = 2^{s-1}-1 = m/2 - 1.$
...
algorithms, meaning that they interpret sequences of length $N = N_1 N_2$ as $N_1 \times N_2$ matrices.
...
Remember in section~\ref{section:omitting1} we established that, with
$x \cdot y$ denoting scalar (element-wise) product between $x,y \in \mathcal{F}^N,$
for any permutation $\pi$ we have $$\pi^{-1}(\pi(x) \cdot \pi(y)) = x \cdot y.$$
